...
void ChessServer::addSession(std::shared_ptr<ChessSession> session) {
    try {
        ...
        { // New scope for the lock
            ...
            // Try to find a game with one player waiting
            std::cout << "[addSession] Searching for a waiting game..." << std::endl;
            for (const auto& pair : games_) {
                std::string current_game_id = pair.first;
                std::shared_ptr<ChessGame> current_game = pair.second;
                
                int players_in_current_game = 0;
                for (const auto& s : sessions_) {
                    if (s->getGameId() == current_game_id) {
                        players_in_current_game++;
                    }
                }

                if (players_in_current_game == 1) { // Found a game with one player
                    assigned_game_id = current_game_id;
                    game = current_game;
                    playerColor = "black"; // Second player is always black
                    shouldStartGame = true;
                    std::cout << "[addSession] Found waiting game: " << assigned_game_id << std::endl;
                    break;
                }
            }

            if (assigned_game_id.empty()) { // No waiting game found, create a new one
                std::cout << "[addSession] No waiting game found. Creating a new one." << std::endl;
                assigned_game_id = "game_" + std::to_string(next_game_id_++);
                game = std::make_shared<ChessGame>();
                games_[assigned_game_id] = game;
                playerColor = "white"; // First player in a new game is always white
                std::cout << "[addSession] Created new game: " << assigned_game_id << std::endl;
            }

            if (!game) {
                std::cerr << "[addSession] CRITICAL ERROR: Game object is null before use!" << std::endl;
                return; // Prevent crash
            }

            session->setPlayerColor(playerColor);
            session->setGameId(assigned_game_id);
            sessions_.push_back(session);

            std::cout << "[addSession] Player connected as " << playerColor << " to game " << assigned_game_id << std::endl;

            // Prepare messages to be sent outside the lock
            playerAssigned_msg["type"] = "playerAssigned";
            playerAssigned_msg["color"] = playerColor;
            playerAssigned_msg["gameId"] = assigned_game_id;
            ...
            if (shouldStartGame) {
                ...
                gameStart_msg["gameId"] = assigned_game_id;
            }
            std::cout << "[addSession] Mutex unlocked, messages prepared." << std::endl;
        } // Mutex is unlocked here

        // Send messages outside the lock using net::post to avoid deadlocks
        net::post(session->get_executor(), [session, msg = playerAssigned_msg.dump()]() {
            std::cout << "[addSession] Sending playerAssigned message." << std::endl;
            session->send(msg);
        });

        net::post(session->get_executor(), [session, msg = boardUpdate_msg.dump()]() {
            std::cout << "[addSession] Sending boardUpdate message." << std::endl;
            session->send(msg);
        });

        if (shouldStartGame) {
            std::cout << "[addSession] Starting game " << assigned_game_id << std::endl;
            broadcastMessage(gameStart_msg.dump(), assigned_game_id);
        }
    }
    catch (const std::exception& e) {
