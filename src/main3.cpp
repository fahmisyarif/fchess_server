...
public:
    ...
    void start() {
        std::cout << "[Session] New session started." << std::endl;
        try {
            ...
            if (login_message.rfind("LOGIN:", 0) == 0) {
                ...
                std::cout << "[Session] User " << this->username << " logged in." << std::endl;
            } else {
                std::cout << "[Session] Invalid login message. Closing." << std::endl;
                return;
            }

            {
                ...
                std::cout << "[Session] " << this->username << " assigned color: " << (int)this->color << std::endl;
            }
            ...
            // Perbarui status game jika ada 2 pemain
            {
                ...
                if (current_game_state == GameState::WAITING_FOR_PLAYERS && players.size() >= 2) {
                    ...
                    if (has_white && has_black) {
                        ...
                        std::cout << "[Game State] Changed to READY_TO_START." << std::endl;
                    }
                }
            }
            ...
            if (se.code() != websocket::error::closed) std::cerr << "[Session] Error: " << se.code().message() << std::endl;
        } catch (std::exception const& e) {
            std::cerr << "[Session] Exception: " << e.what() << std::endl;
        }
        std::cout << "[Session] Session for " << this->username << " ended." << std::endl;
    }

private:
    void read_loop() {
        std::cout << "[Session] Read loop started for " << this->username << std::endl;
        try {
            for (;;) {
                beast::flat_buffer buffer;
                ws.read(buffer);
        ...
        std::cout << "[Session] Read loop ended for " << this->username << std::endl;
        cleanup();
    }

    void handle_message(const std::string& message_str) {
        std::cout << "[Handle Message] Received: " << message_str << " from " << this->username << std::endl;
        std::lock_guard<std::mutex> lock(board_mutex);

        // Handle START_GAME command
        if (message_str == "START_GAME") {
            std::cout << "[Handle Message] START_GAME command received." << std::endl;
            ...
            if (current_game_state == GameState::READY_TO_START) {
                ...
                std::cout << "[Handle Message] Game state set to IN_PROGRESS." << std::endl;
            } else {
                send_message("ERROR:Game cannot be started now.");
                std::cout << "[Handle Message] Game not ready to start." << std::endl;
            }
            return;
        }

        // Handle RESIGN command
        if (message_str == "RESIGN") {
            std::cout << "[Handle Message] RESIGN command received." << std::endl;
            std::lock_guard<std::mutex> gs_lock(game_state_mutex);
            if (current_game_state == GameState::IN_PROGRESS) {
                current_game_state = GameState::GAME_OVER;
                std::string resign_msg = "FEN:" + board.getFen() + "|GAMEOVER:" + this->username + " resigned.";
                broadcast(resign_msg);
                broadcast_game_state();
                std::cout << "[Handle Message] " << this->username << " resigned. Game Over." << std::endl;
            } else {
                send_message("ERROR:Cannot resign now.");
                std::cout << "[Handle Message] Cannot resign, game not in progress." << std::endl;
            }
            return;
        }
        ...
        // Hanya izinkan gerakan jika game sedang berjalan
        {
            ...
            if (current_game_state != GameState::IN_PROGRESS) {
                ...
                std::cout << "[Handle Message] Move rejected: Game not in progress." << std::endl;
                return;
            }
        }
        ...
        if (this->color == chess::Color::NONE) {
            ...
            std::cout << "[Handle Message] Move rejected: Spectator." << std::endl;
            return;
        }
        ...
        if (board.sideToMove() != this->color) {
            ...
            std::cout << "[Handle Message] Move rejected: Not " << this->username << "'s turn." << std::endl;
            return;
        }
        ...
        if (!parseUci(message_str, move)) {
            ...
            std::cout << "[Handle Message] Move rejected: Invalid format." << std::endl;
            return;
        }
        ...
        if (move_is_legal) {
            ...
            // Fixed game over check - use proper pair access
            auto game_over_info = board.isGameOver();
            if (game_over_info.first != chess::GameResultReason::NONE) {
                fen_msg += "|GAMEOVER:";
                auto reason = game_over_info.first; // Access the GameResultReason (first element)

                {
                    std::lock_guard<std::mutex> gs_lock(game_state_mutex); // Lock game state before modifying
                    if (reason == chess::GameResultReason::CHECKMATE) {
                        ...
                        fen_msg += "Draw by Insufficient Material.";
                        current_game_state = GameState::GAME_OVER;
                    }
                    else if (reason == chess::GameResultReason::THREEFOLD_REPETITION) {
                        fen_msg += "Draw by Threefold Repetition.";
                        current_game_state = GameState::GAME_OVER;
                    }
                    else {
                        fen_msg += "Game Over.";
    ...
    void send_message(const std::string& message) {
        try {
            ...
            std::cout << "[Send Message] Sent to " << this->username << ": " << message << std::endl;
        } catch (...) { /* ignore */
            std::cerr << "[Send Message] Error sending to " << this->username << std::endl;
        }
    }
    ...
    void send_current_board_state() {
        ...
        std::cout << "[Session] Sent initial FEN to " << this->username << std::endl;
    }

    void cleanup() {
        std::cout << "[Cleanup] Cleaning up session for " << this->username << std::endl;
        std::lock_guard<std::mutex> lock(players_mutex);
        ...
        // Reset game state if not enough players
        {
            ...
            if (players.size() < 2 && current_game_state != GameState::WAITING_FOR_PLAYERS) {
                ...
                std::cout << "[Cleanup] Game state reset to WAITING_FOR_PLAYERS." << std::endl;
                ...
                if (current_game_state == GameState::IN_PROGRESS || current_game_state == GameState::GAME_OVER) {
                    ...
                    std::cout << "[Cleanup] Board reset to start position." << std::endl;
                }
            }
        }
        ...
        broadcast_player_list();
        broadcast_game_state();
        std::cout << "[Cleanup] Broadcasted player list and game state." << std::endl;
    }
...
void broadcast(const std::string& message) {
    ...
    for (const auto& p : players_copy) {
        try {
            ...
            std::cerr << "[Broadcast] Error sending to " << p->username << std::endl;
        }
    }
}
